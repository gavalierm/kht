...
class GameInstance {
  constructor(gamePin, questions, dbId = null) {
    ...
    this.playerJoinOrder = 0; // Track join order for consistent player numbering
  }
  ...
  addPlayer(playerId, playerData) {
    ...
    if (existingPlayer) {
      // Player reconnecting - keep existing name and join order
      existingPlayer.score = playerData.score || existingPlayer.score;
      existingPlayer.connected = true;
      existingPlayer.token = playerData.player_token || existingPlayer.token;
      return;
    }
