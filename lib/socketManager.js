...
class SocketManager {
  ...
  canJoinGame(gamePin, connectionType = 'player') {
    ...
    // For moderators and panels, they have separate higher limits even if players are full
    if (connectionType === 'moderator' || connectionType === 'panel') {
      return currentConnections < this.maxPlayersPerGame + limits[connectionType];
    }
  ...
  joinGame(socket, gamePin, connectionType = 'player') {
    try {
      ...
      console.error(`Error joining game ${gamePin}:`, error);
      return false;
    }
  }
  ...
  executeBatchedBroadcasts(broadcasts) {
    try {
      ...
      console.error('Error executing batched broadcasts:', error);
    }
  }
  ...
  queueDatabaseOperation(operation) {
    try {
      ...
      console.error('Error queuing database operation:', error);
    }
  }
  ...
  processBatchedOperations() {
    try {
      ...
      console.error('Error processing batched operations:', error);
    }
  }
